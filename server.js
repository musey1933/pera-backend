...
app.get('/api/phones/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { ...phone, specs: specsObj, perf: perf || {}, stores } });
  } catch (err) {
...
app.post('/api/alarms', async (req, res) => {
  try {
    ...
    const [[existing]] = await db.query(
      'SELECT id FROM price_alarms WHERE phone_id=? AND email=? AND triggered=0', [phoneId, email]
    );
    if (existing) return res.status(400).json({ error: 'Bu ürün için zaten aktif alarmınız var.' });

    await db.query('INSERT INTO price_alarms (phone_id, target_price, email) VALUES (?,?,?)', [phoneId, targetPrice, email]);
    ...
    try {
      await mailer.sendMail({
        ...
        html: `<div style="font-family:sans-serif;max-width:500px"><h2 style="color:#1a3fd4">PERA Fiyat Alarmı</h2><p><strong>${phone.name}</strong> için fiyat alarmınız kuruldu.</p><p>Hedef fiyat: <strong>${Number(targetPrice).toLocaleString('tr-TR')}₺</strong></p><hr><small style="color:#888">getpera.tr</small></div>`
      });
    } catch (mailErr) {
...
app.get('/api/alarms', authMiddleware, async (req, res) => {
  try {
    const [alarms] = await db.query(`
      ...
      FROM price_alarms pa JOIN phones p ON p.id = pa.phone_id
      WHERE pa.email = ? ORDER BY pa.created_at DESC
    `, [req.user.email]);
...
// ── FİYAT ALARMI KONTROL JOB'U ────────────────────────────────────────────────
setInterval(async () => {
  try {
    const [alarms] = await db.query(`
      SELECT pa.*, p.name as phone_name, p.price as current_price
      FROM price_alarms pa JOIN phones p ON p.id = pa.phone_id
      WHERE pa.triggered = 0 AND p.price <= pa.target_price
    `);
    for (const alarm of alarms) {
      try {
        await mailer.sendMail({
          ...
          html: `<div style="font-family:sans-serif;max-width:500px"><h2 style="color:#059669">🔔 Fiyat Alarmı Tetiklendi!</h2><p><strong>${alarm.phone_name}</strong> hedef fiyata ulaştı!</p><a href="https://getpera.tr" style="background:#1a3fd4;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:12px">Hemen Satın Al →</a><hr><small style="color:#888">getpera.tr</small></div>`
        });
...
app.get('/api/filters', async (req, res) => {
  try {
    const [ramRows] = await db.query(`SELECT spec_val as val, COUNT(*) as count FROM phone_specs WHERE spec_key='RAM' GROUP BY spec_val ORDER BY CAST(spec_val AS UNSIGNED)`);
    const [storageRows] = await db.query(`SELECT spec_val as val, COUNT(*) as count FROM phone_specs WHERE spec_key='Depolama' GROUP BY spec_val ORDER BY CAST(spec_val AS UNSIGNED)`);
    const [storeRows] = await db.query('SELECT store_name as val, COUNT(*) as count FROM store_prices GROUP BY store_name ORDER BY count DESC');
    const [[priceRange]] = await db.query('SELECT MIN(price) as minP, MAX(price) as maxP FROM phones');
    res.json({ success: true, data: { ram: ramRows, storage: storageRows, stores: storeRows, priceRange } });
  } catch (err) {
    res.status(500).json({ error: 'Sunucu hatası' });
  }
});
...
app.listen(PORT, async () => {
  ...
  // Tabloları ve örnek veriyi otomatik kur
  try {
    await setup();
  } catch(err) {
